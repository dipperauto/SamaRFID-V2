...
def delete_user(username: str) -> bool:
    """
    Remove usuário do CSV e apaga a foto de perfil se existir.
    """
    _ensure_csv()
    rows = []
    removed = False
    ...
    with open(USERS_CSV_PATH, "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            if row.get("username") == username:
                removed = True
                photo_to_delete = row.get("profile_photo_path") or ""
                continue
            rows.append(row)
    if not removed:
        return False
    ...
    try:
        if photo_to_delete:
            p = photo_to_delete.replace("\\", "/")
            if p.startswith("static/"):
                p = p.replace("static/", "media/")
